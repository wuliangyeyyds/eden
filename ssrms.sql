...
CREATE TABLE `feedback` (
  ...
  `category`        VARCHAR(30)  NOT NULL COMMENT 'env/complaint/appeal 等',
  `rating`          INT          NULL COMMENT '1-5 分',
  ...
  PRIMARY KEY (`id`),
  KEY `idx_fb_user_id` (`user_id`),

  CONSTRAINT `fk_fb_user`
    FOREIGN KEY (`user_id`) REFERENCES `user`(`id`)
    ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='评价与投诉表';
...
CREATE TABLE `notice` (
  ...
  `content`     TEXT         NOT NULL,
  `level`       VARCHAR(20)  NOT NULL DEFAULT 'normal' COMMENT 'important/normal',
  `target_role` VARCHAR(20)  NOT NULL DEFAULT 'all' COMMENT 'all/user/admin',
